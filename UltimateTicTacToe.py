...
# Authors - Vraj Patel U28629231, Daishak Patel U
...
def printInstructions():
    ...
    print("9.  If the overall board is tied, then the player with the most regions wins.")
...
class UltimateTicTacToe:
    ...
    def determine_depth(self):
        if self.move_counter <= 3:
            return 0
        elif 4 <= self.move_counter <= 10:
            return 2
        elif 11 <= self.move_counter <= 60:
            return 4
        else:
            return 6
        
        
    # displays the board in the form of an adjacency matrix displaying which vertices are connected by which edges
    # since board is an undirected graph, this matrix is symmetrical along the diagonal
    ...
    # annotates the matrix to show vertices v1 and v2 are connected
    ...
    # return 1 if Player 1 has won, return 2 if Player 2 has won, returns 0 if game still on
    ...
    # NOTE TO SELF: Remove this function, temporary to help randomly make a choice
    ...
    def find_best_move(self, player):
        ...
        for move in legal_moves:
            ...
            nodesSearched += nodes
            #print("Move:",move[0]+1,move[1]+1,"Value:",move_val)
        ...
        #print("Best move:",best_move[0]+1,best_move[1]+1,"Best move value:",best_val)
    ...
    # evaluation function used to evaluate a board once depth limit is reached
    def evaluate_board(self,player):
        ...
        if player == 1: # computer is player 1 ('X')
            ...
            score += self.largeBoard.count('X')*4.5
            score -= self.largeBoard.count('O')*4.5
            # Positions Heuristic - number of opportunities
            opportunities_X, opportunities_O = self.count_opportunities()
            score += opportunities_X*0.25
            score -= opportunities_O*0.25
    ...
    # finds utility of a possible move using minimax, player is always the computer and is maximizing
    def evaluate_move(self, depth, is_maximizing, player, alpha, beta, nodesSearched=0):
        ...
        if depth == 0:     
            return self.evaluate_board(player), nodesSearched

        # implements minimax depending on if the current player is maximizing or minimizing 
        if is_maximizing:
            max_eval = float('-inf')
            legal_moves = self.determine_legal_moves(player)
            for move in legal_moves:
...
# Call the function to wait for any key press, clear after 
